...
- 底部 4 Tab：`Home / Learn / Dict / Me`
- Home 二级 Tab：`练习 / 今日一字`（每日入口不再挤压笔顺/画布区域；从“今日一字”开始练会自动切回练习）
- 练习页改造：画布与卡片走 `MaterialTheme.colorScheme`，底部 3 动作条（发音 / 主按钮 / 字典），低频入口收拢到右上角设置
- 主题系统：统一圆角 `Shapes` + 中性灰阶/强调色；新增“跟随系统/浅色/深色”设置项
- 音量安全：发音前高音量提醒（阈值/降到安全档可配置），并支持设置开关
- 新手引导：首次启动引导页，并支持在 `Me` 页再次打开
